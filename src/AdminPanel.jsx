...
const AdminPanel = () => {
  ...
  const fetchLeads = () => {
    const token = localStorage.getItem("adminToken");
    axios
      .get("https://properties-backend-ok36.onrender.com/api/leads", {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const deleteLead = (id) => {
    ...
    const token = localStorage.getItem("adminToken");
    axios
      .delete(`https://properties-backend-ok36.onrender.com/api/leads/${id}`,
        { headers: { Authorization: `Bearer ${token}` } })
      .then(() => fetchLeads())
      .catch(() => {
        alert("Session expired. Please login again.");
              ...
              <tbody>
                {leads.map((lead, i) => (
                  <tr key={i} className="border-t text-center">
                    ...
                    <td className="px-3 py-2">{lead.propertyId}</td>
